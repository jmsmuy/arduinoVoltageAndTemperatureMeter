...
void SevSeg::setTemperature(long numToShow, char decPlaces, bool hex) //long
{
  setNewNum(numToShow, decPlaces, hex, true);
}

void SevSeg::setNumber(long numToShow, char decPlaces, bool hex) //long
{
  setNewNum(numToShow, decPlaces, hex, false);
}
...
void SevSeg::setNewNum(long numToShow, char decPlaces, bool hex, bool isTemp) {
  byte digits[numDigits];
  findDigits(numToShow, decPlaces, hex, digits, isTemp);
...
void SevSeg::findDigits(long numToShow, char decPlaces, bool hex, byte digits[], bool isTemp) {
  ...
  if (numToShow > maxNum || numToShow < minNum) {
    ...
    if(isTemp){
      byte auxDigit = digits[0];
      for(int i = 0; i < numDigits - 1; i++){
        digits[i] = digits[i + 1];
      }
      digits[numDigits - 1] = DEGREE_SYMBOL;
    }
